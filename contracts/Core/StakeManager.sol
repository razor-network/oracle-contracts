...
contract StakeManager is Utils, ACL, StakeStorage {
    ...
    function setStakerEpochLastRevealed(uint256 _id, uint256 _epochLastRevealed)
        external
        onlyRole(Constants.getStakerActivityUpdaterHash())
    {
    ...
    function stake(uint256 epoch, uint256 amount)
        external
        checkEpoch(epoch)
        checkState(Constants.commit())
    {
        // not allowed during reveal period
        require(
            stateManager.getState() != Constants.reveal(),
            "Incorrect state"
        );
        require(
        ...
        if (stakerId == 0) {
            ...
            stakers[numStakers] = Structs.Staker(
                ...
                0,
                0,
                epoch.add(Constants.unstakeLockPeriod()),
                0
            );
            stakerId = numStakers;
            ...
            stakers[numStakers].unstakeAfter = epoch.add(
                Constants.unstakeLockPeriod()
            );
    ...
    /// @notice staker must call unstake() and then wait for Constants.WITHDRAW_LOCK_PERIOD
    /// Please note Staker must Not vote in follwoing epochs after unstake
    ...
    function unstake(uint256 epoch)
        external
        checkEpoch(epoch)
        checkState(Constants.commit())
    {
        ...
        require(
            staker.unstakeAfter <= epoch && staker.unstakeAfter != 0,
            "locked"
        );
        ...
        checkState(Constants.commit())
    {
        ...
        require(
            (staker.withdrawAfter - Constants.withdrawLockPeriod()) >=
    ...
    function givePenalties(uint256 stakerId, uint256 epoch)
        external
        onlyRole(Constants.getStakeModifierHash())
    {
    ...
    function _calculateBlockReward() private returns (uint256) {
        uint256 halvings =
            (block.number - genesisBlock) / Constants.halvingInterval();
        ...
        if (lowerCutoffsLastEpoch.length > 0) {
            ...
            if (reward > 0) {
                ...
                stakeGettingReward = stakeGettingReward >= thisStaker.stake
                    ? stakeGettingReward.sub(thisStaker.stake)
                    : 0;
                emit StakeGettingRewardChange(
                    epoch,
                    prevStakeGettingReward,
        ...
        returns (uint256)
    {
        ...
        uint256 penalty =
            (epochs.sub(1)).mul(
                (stakeValue.mul(Constants.penaltyNotRevealNum())).div(
        ...
        internal
    {
        ...
        uint256 currentStake =
            calculateInactivityPenalties(penalizeEpochs, previousStake);
        ...
        if (lowerCutoffsLastEpoch.length > 0) {
            ...
            for (uint256 i = 0; i < lowerCutoffsLastEpoch.length; i++) {
                ...
                if (
                    ...
                    penalty =
                        penalty +
                        (previousStake / Constants.exposureDenominator());
                }
            }
