...
contract AssetManager is ACL, AssetStorage {
    ...
    function updateJob(
        ...
        require(
            jobs[jobID].assetType == uint256(assetTypes.Job),
            "Job ID not present"
        );
    ...
    function setAssetStatus(uint256 id, bool assetStatus)
        external
    ...
    function fulfillAsset(uint256 id, uint256 value)
        external
        onlyRole(parameters.getAssetConfirmerHash())
    {
        ...
        if (jobs[id].assetType == uint256(assetTypes.Job)) {
            ...
            collections[id].assetType == uint256(assetTypes.Collection)
        ) {
    ...
    function createCollection(
        ...
        require(
            aggregationMethod > 0 &&
                aggregationMethod < parameters.aggregationRange(),
            "Aggregation range out of bounds"
        );
        ...
        for (uint256 i = 0; i < jobIDs.length; i++) {
            require(
                jobs[jobIDs[i]].assetType == uint256(assetTypes.Job),
                "Job ID not present"
            );
            require(jobs[jobIDs[i]].active, "Job ID not active");
            require(
                !collections[numAssets].jobIDExist[jobIDs[i]],
                "Duplicate JobIDs sent"
            );
    ...
    function addJobToCollection(uint256 collectionID, uint256 jobID)
        external
        onlyRole(parameters.getAssetModifierHash())
    {
        require(
            collections[collectionID].assetType ==
                uint256(assetTypes.Collection),
            "Collection ID not present"
        );
        ...
        require(
            !collections[collectionID].jobIDExist[jobID],
            "Job exists in this collection"
        );
    ...
    function removeJobFromCollection(uint256 collectionID, uint256 jobIDIndex)
        external
        onlyRole(parameters.getAssetModifierHash())
    {
        require(
        ...
        require(
            collections[collectionID].jobIDs.length > jobIDIndex,
            "Index not in range"
        );
        ...
        for (
            uint256 i = jobIDIndex;
            i < collections[collectionID].jobIDs.length - 1;
            i++
        ) {
            collections[collectionID].jobIDs[i] = collections[collectionID]
                .jobIDs[i + 1];
        }
    ...
    {
        require(
            jobs[id].assetType == uint256(assetTypes.Job),
            "ID is not a job"
        );
