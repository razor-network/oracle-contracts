...
contract BlockManager is Initializable, BlockStorage, StateManager, BlockManagerParams, IBlockManager {
    ...
    function propose(
        ...
        uint256 iteration,
        uint32 biggestStakerId
    ) external initialized checkEpochAndState(State.Propose, epoch, buffer, epochLength) {
    ...
    ) external initialized checkEpochAndState(State.Dispute, epoch, buffer, epochLength) {
     ...
     * @param epoch in which the dispute was setup and raised
     */
    function resetDispute(uint32 epoch) external initialized checkEpochAndState(State.Dispute, epoch, buffer, epochLength) {
     ...
     */
    function claimBlockReward() external initialized checkState(State.Confirm, buffer, epochLength) {
