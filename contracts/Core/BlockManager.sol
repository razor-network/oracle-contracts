...
contract BlockManager is Initializable, ACL, BlockStorage {
    ...
    function propose(
        ...
        require(
            isElectedProposer(iteration, biggestInfluencerId, proposerId),
            "not elected"
        );
        require(
        ...
        uint256 biggestInfluence = stakeManager.getInfluence(
            biggestInfluencerId
        );
    ...
    function giveSorted(
        ...
        uint256 medianWeight = voteManager.getTotalInfluenceRevealed(
            epoch,
            assetId
        ) / (2);
        ...
        if (disputes[epoch][msg.sender].accWeight == 0) {
            ...
            require(
                disputes[epoch][msg.sender].assetId == assetId,
                "AssetId not matching"
            );
        }
        for (uint256 i = 0; i < sorted.length; i++) {
            require(
                sorted[i] > lastVisited,
                "sorted[i] is not greater than lastVisited"
            );
            lastVisited = sorted[i];
            accWeight =
                accWeight +
                (voteManager.getVoteWeight(epoch, assetId, sorted[i]));

            if (
                disputes[epoch][msg.sender].median == 0 &&
                accWeight > medianWeight
            ) {
    ...
    function resetDispute(uint256 epoch)
        external
        initialized
        checkEpoch(epoch)
        ...
        checkState(parameters.dispute())
    {
        ...
        require(
            disputes[epoch][msg.sender].accWeight ==
                voteManager.getTotalInfluenceRevealed(epoch, assetId),
        ...
        onlyRole(parameters.getBlockConfirmerHash())
    {
        ...
        for (uint8 i = 0; i < proposedBlocks[epoch - 1].length; i++) {
            if (proposedBlocks[epoch - 1][i].valid) {
                ...
                for (
                    uint8 j = 0;
                    j < proposedBlocks[epoch - 1][i].ids.length;
                    j++
                ) {
                    assetManager.fulfillAsset(
                        proposedBlocks[epoch - 1][i].ids[j],
    ...
    function getBlock(uint256 epoch)
        external
        view
    ...
    function getBlockMedians(uint256 epoch)
        external
        view
        returns (uint256[] memory _blockMedians)
    {
    ...
    function isElectedProposer(
        ...
        bytes32 randHash = Random.prngHash(
            10,
            keccak256(abi.encode(stakerId, iteration)),
            parameters.epochLength()
        );
    ...
    function _insertAppropriately(uint256 epoch, Structs.Block memory _block)
        internal
    {
        ...
        for (uint256 i = 0; i < proposedBlocks[epoch].length; i++) {
            if (
                proposedBlocks[epoch][i].biggestInfluence <
                _block.biggestInfluence
            ) {
                pushAt = i;
                break;
            }
