...
contract VoteManager is Initializable, ACL, VoteStorage {
    ...
    function commit(uint256 epoch, bytes32 commitment)
        external
        initialized
        checkEpoch(epoch)
        checkState(parameters.commit())
    {
        ...
        if (
            blockManager.getBlock(epoch - 1).proposerId == 0 &&
            blockManager.getNumProposedBlocks(epoch - 1) > 0
        ) {
    ...
    function reveal(
        ...
        require(
            ...
            "not commited or already revealed"
        );
        require(
            keccak256(abi.encodePacked(epoch, root, secret)) ==
                commitments[epoch][thisStakerId],
        ...
        if (msg.sender == stakerAddress) {
            ...
            for (uint256 i = 0; i < values.length; i++) {
                require(
                    MerkleProof.verify(
                        proofs[i],
                        root,
                        keccak256(abi.encodePacked(values[i]))
                    ),
                    "invalid merkle proof"
                );
                uint256 influence = stakeManager.getInfluence(thisStakerId);
                votes[epoch][thisStakerId][i] = Structs.Vote(
                    values[i],
                    thisStaker.stake
                );
                voteWeights[epoch][i][values[i]] =
                    voteWeights[epoch][i][values[i]] +
                    influence;
                totalInfluenceRevealed[epoch][i] =
                    totalInfluenceRevealed[epoch][i] +
                    influence;
            }
            ...
            require(
                parameters.getState() == parameters.commit(),
                "Not commit state"
            );
            commitments[epoch][thisStakerId] = 0x0;
    ...
    function getCommitment(uint256 epoch, uint256 stakerId)
        public
        view
        returns (bytes32)
    {
    ...
    function getTotalInfluenceRevealed(uint256 epoch, uint256 assetId)
        public
        view
        returns (uint256)
    {
