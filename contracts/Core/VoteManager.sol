...
contract VoteManager is Initializable, ACL, VoteStorage {
    ...
    function reveal(
        ...
        if (assetManager.getNumActiveAssets() != 0){
                ...
                for (uint256 i = 0; i < values.length; i++) {
                    if (votes[epoch][thisStakerId][values[i].id - 1].weight == 0) {
                        ...
                        voteWeights[epoch][values[i].id - 1][values[i].value] =
                            voteWeights[epoch][values[i].id - 1][values[i].value] +
                            influence;
                        if (totalInfluenceRevealed[epoch][values[i].id - 1] == 0) numRevealedAssets[epoch]++;
    ...
    function isAssetAllotedToStaker(
        ...
        uint256[] memory activeAssets = assetManager.getActiveAssets();
        if (
            activeAssets[(Random.prng(10, assetManager.getNumActiveAssets(), keccak256(abi.encode(iteration + stakerId)), parameters.epochLength()))] ==
            assetId
        ) return true;
        return false;
    }
    ...
    function getNumRevealedAssets(uint256 epoch) public view returns (uint256) {
        return numRevealedAssets[epoch];
    }
}
