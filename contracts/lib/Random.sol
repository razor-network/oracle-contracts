...
library Random {
    ...
    function blockHashes(uint8 numBlocks, uint256 epochLength)
        public
        view
        returns (bytes32)
    {
        ...
        uint256 blockNumberEpochStart = (block.number / (epochLength)) *
            (epochLength);
        for (uint8 i = 1; i <= numBlocks; i++) {
            sum = keccak256(
                abi.encodePacked(sum, blockhash(blockNumberEpochStart - i))
            );
        }
        return (sum);
    }
