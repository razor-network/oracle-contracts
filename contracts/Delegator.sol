...
contract Delegator is ACL, Constants, IDelegator {
    ...
    function getNumActiveAssets() external override view returns (uint256) {
        return assetManager.getNumActiveAssets();
    }

    function getActiveAssets() external override view returns (uint8[] memory) {
        return assetManager.getActiveAssets();
    }

    function getResult(bytes32 _name) external override view returns (uint32, int8) {
