...
describe('AssignCollectionsRandomly', function () {
  ...
  describe('razor', async () => {
    ...
    it('Staker Proposes Everything correctly, none of dispute should go through', async () => {
      ...
      // propose [ 1, 2, 6, 7 ] [ 100, 200, 600, 700 ]
    ...
    it('Staker Proposes with missing id', async () => {
      ...
      snapshotId = await takeSnapshot();
      // missing 3
      await adhocPropose(signers[1], [1, 6, 7], [100, 600, 700], stakeManager, blockManager, voteManager);
      await mineToNextState();
      const epoch = await getEpoch();
    ...
    it('Staker Proposes with additional id', async () => {
      ...
      snapshotId = await takeSnapshot();
      // additional 2
      await adhocPropose(signers[1], [1, 2, 3, 6, 7], [100, 200, 300, 600, 700], stakeManager, blockManager, voteManager);
      await mineToNextState();
      const epoch = await getEpoch();
